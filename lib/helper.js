...
async function injectScript (options = {}) {
  ...
  // appJs
  const appJs = this.app.waibuMpa.config.asset.appJs
  if (appJs) regular.push(printScript.call(this, { src: appJs }))
...
async function injectMeta (options = {}) {
  ...
  // favicon
  const faviconPng = this.app.waibuMpa.config.asset.faviconPng
  if (faviconPng) items.push({ tag: 'link', href: faviconPng, rel: 'icon', type: 'image/png' })
  // meta
  items = map(items, m => {
...
async function injectCss (options = {}) {
  ...
  // appCss
  const appCss = this.app.waibuMpa.config.asset.appCss
  if (appCss) regular.push(printLink.call(this, { href: appCss }))
